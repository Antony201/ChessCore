...
@patch('api.broadcast.services.CreateSession.do_request', return_value=12345)
@patch('api.broadcast.services.CreateData.do_request', return_value=12345)
@patch('api.broadcast.services.CreateRoom.do_request', return_value=12345)
class TestAPI(APITestCase):

    def setUp(self):
        User = get_user_model()
        self.user_one = User.objects.create_user(
            username="whitey_morgan", password="django", email="test_1@email.com",
        )
        self.user_two = User.objects.create_user(
            username="blackie_lawless", password="django", email="test_2@email.com",
        )
    ...
    def _get_game(self, uuid, user):
        get_request = factory.get(f"/api/game/{uuid}/")
    ...
    def _join_game(self, uuid, user, preferred_color="random"):
        ...
        join_request = factory.put(f"/api/game/{uuid}/join/", data)
    ...
    def _move_piece(self, uuid, move, user):
        request = factory.put(f"/api/game/{uuid}/move/", move)
        force_authenticate(request, user=user)
        response = self.move_piece_view(request, pk=uuid, user=user)

        return response

    def _get_unfinished_games(self, user):
        request = factory.get("/api/game/")
        force_authenticate(request, user=user)
        response = self.unfinished_game_view(request)

        return response

    def test_can_create_game(self, *args, **kwargs):
        """
        Create a game
        """
    ...
    def test_can_retrieve_game(self, *args, **kwargs):
        """
        Retrieve a game
        """
    ...
    def test_can_join_game(self, *args, **kwargs):
        """
    ...
    def test_players_can_move_their_pieces(self, *args, **kwargs):
    ...
    def test_players_can_only_move_pieces_they_own(self, *args, **kwargs):
        response = self._create_game(preferred_color="white")
        ...
        self.assertEqual("That move is not valid or allowed", response.data["errors"][0]["detail"])

    def test_only_valid_moves_are_allowed(self, *args, **kwargs):
        response = self._create_game(preferred_color="white")
    ...
    def test_non_player_users_cant_play(self, *args, **kwargs):
        response = self._create_game(preferred_color="white")
    ...
    def test_users_can_move_against_themselves(self, *args, **kwargs):
        # Create game
        response = self._create_game()
        ...
        move_response = self._move_piece(response.data.get("uuid"), move, self.user_one)

        self.assertEqual(game_uuid, move_response.data.get("uuid"))

    def test_game_end(self, *args, **kwargs):
        moves = []
    ...
    def test_can_get_unfinished_games(self, *args, **kwargs):
        game_response = self._create_game(preferred_color="white")
